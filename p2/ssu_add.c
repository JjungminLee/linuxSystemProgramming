...
int check_already_add(dirNode *dirNode, char *absolutePath,
                      char *relativePath)
{
  if (dirNode == NULL)
    return 0;
  ...
  while (dirNode != NULL)
  {
    ...
    if (check_already_add(dirNode, absolutePath, relativePath) == 1)
    {
      return 1;
    }
...
int ssu_add(char *path)
{
  ...
  if (check_already_add(staging_dir_list, absolutePath, relativePath) == 1)
  {
    return 1;
  }
  // log와 표준출력쓰기
