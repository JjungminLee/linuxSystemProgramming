...
void backupNode_init(backupNode *backup_node)
{
  ...
  memset(backup_node->time, 0, sizeof(backup_node->time));
  memset(backup_node->pid, 0, sizeof(backup_node->pid));

  backup_node->prev_backup = NULL;
  backup_node->next_backup = NULL;
}

backupNode *backupNode_get(backupNode *backup_head, char *time)
{
...
backupNode *backupNode_insert(backupNode *backup_head, char *time,
